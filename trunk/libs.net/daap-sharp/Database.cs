...
namespace DAAP {
    ...
    public class Database : ICloneable {
        ...
        //private Metadata trackData;
        private Playlist basePlaylist = new Playlist (0);
        ...
        public IList<TrackBase> Tracks {
            get {
                Metadata trackData = new Metadata();
                return new ReadOnlyCollection<TrackBase> (trackData.TrackList);
            }
        }
        ...
        public int TrackCount {
            get {
                Metadata trackData = new Metadata();
                return trackData.SongCount; }
        }

        public Track TrackAt(int index)
        {
            Metadata trackData = new Metadata();
            return new Track(trackData.GetSong(index));
        }

        /*
        public IList<PlaylistBase> Playlists {
            get {
        ...
        private Database () {
            ...
            Metadata plMetadata = new Metadata();

            basePlaylist = new Playlist(plMetadata.GetPlaylist(0));
            basePlaylist.TrackMetaHandler = LookupTrackById;

            //this.tracks = trackData.TrackList;
            //this.trackData = new Metadata();
        }
        ...
        public Database (string name) : this () {
            ...
            //this.tracks = trackData.TrackList;
            //this.trackData = new Metadata();
            //trackData.AddDirectory("H:\\home\\Music\\Radiohead");
            //trackData.UpdateMusic();
        }
        ...
        public Track LookupTrackById (int id) {
            Metadata trackData = new Metadata();
            return new Track(trackData.GetSong(id));
        }
        ...
        internal ContentNode ToTracksNode (string[] fields, int[] deletedIds, int revision) {

            ArrayList trackNodes = new ArrayList ();
            Metadata trackData = new Metadata();
            foreach (TrackBase track in trackData.GetTracks(revision)) {
                //Track temp = new Track(track);
                trackNodes.Add (DAAP.Track.ToNode (track,fields));
            }
            ...
            children.Add (new ContentNode ("dmap.specifiedtotalcount", trackData.SongCount));
            children.Add (new ContentNode ("dmap.returnedcount", trackData.SongCount));
        ...
        internal ContentNode ToDatabaseNode () {
            ...
            return new ContentNode ("dmap.listingitem",
                                    ...
                                    new ContentNode ("dmap.itemcount", trackData.SongCount),
        ...
        private void ClearTracks () {
            Metadata trackData = new Metadata();
            foreach (TrackBase track in new List<TrackBase> (trackData.TrackList)) {
                RemoveTrack (new Track(track));
            }
        }
        ...
        private void RefreshPlaylists (string revquery) {
            ...
            foreach (Playlist pl in playlists) {
                ...
                foreach (ContentNode plTrackNode in (ContentNode[]) playlistTracksNode.GetChild ("dmap.listing").Value) {
                    ...
                    if (pl[plindex] == null) {
                        pl.InsertTrack (plindex, pltrack);
                    }
        ...
        /// <summary>
        /// Client side DB function that refreshes tracks from server
        /// </summary>
        ...
        private Playlist ClonePlaylist (Database db, Playlist pl) {
            Playlist clonePl = (Playlist) pl.Clone();


            /*
            List<Int32> pltracks = pl.Tracks;
        ...
        public object Clone () {
            ...
            /*
            List<TrackBase> cloneTracks = new List<TrackBase> ();
            foreach (Track track in trackData.TrackList) {
